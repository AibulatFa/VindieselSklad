...
@Composable
fun RDensoScreen(navController: NavController) {
    var productList by remember { mutableStateOf(emptyList<Product>()) }

    LaunchedEffect(Unit) {
        val list = getProductList2()
        productList = list
    }
...
suspend fun getProductList2(): List<Product> {
    val firestore = FirebaseFirestore.getInstance()
    val productList = mutableListOf<Product>()

    val collection = firestore.collection("Rdenso")
